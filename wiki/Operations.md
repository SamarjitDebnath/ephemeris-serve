...
- **Stop-sequence detection decodes a bounded window, not the full history**: matching is still on decoded *text* rather than token ids, since a stop sequence can span multiple tokens and needn't align with token boundaries -- but `ContinuousScheduler._dispatch_tokens()` decodes only the last `max_stop_length + stop_window_slack_tokens` tokens as a filter, and pays for a full decode once, on the step that actually stops, to resolve the result at the right offset. `streaming/stream_manager.py` uses two-offset incremental detokenization for the same reason. Decoding is linear in token count (~0.25 us/token), so the previous whole-history decode made a request quadratic in its own output length: ~124 ms of pure decode over a 1024-token generation, against ~9 ms windowed.
- **Metrics come in two shapes**: `GET /api/metrics` returns rolling averages as JSON (the CLI consumes it) and `GET /metrics` returns raw Prometheus counters/gauges/histograms when `metrics_config.prometheus_enabled` is set and the optional `metrics` extra is installed. Neither derives from the other -- the JSON averages over the last 1000 samples, a window whose *duration* varies with traffic, which is precisely what a time-series backend must not be handed pre-aggregated. See `deploy/prometheus/README.md` for scrape config, the per-gauge multiprocess modes, and what a bad value looks like for each metric.
- **Model swap is single-process unless `scheduler_config.model_state_dir` is set**: `POST /api/model` reloads only the process that handles the request. With a state directory configured, the handling worker publishes a new generation and every other worker converges on its next idle tick. The swap is deliberately not atomic across workers -- each drains its own in-flight requests first -- so `ModelSwapResponse` reports `generation`, `converged_workers`, and `known_workers`, and a client polls `GET /api/model` until the counts match.
- **Two earlier prototypes were replaced, and the reasons still matter**: a fixed-size batch scheduler with no paged cache, superseded by `scheduler/batch_scheduler.py` and `scheduler/continuous_scheduler.py` once per-request KV state had to outlive a single batch; and an SSE streamer that re-decoded the whole token history per token, superseded by `streaming/stream_manager.py`. The prototypes themselves are not in the repository. Note that the second one's cost is not purely historical -- see the full-history decode entry above, which is the same O(n²) shape still present in the live streamer.
